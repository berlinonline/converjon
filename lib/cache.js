...
function cache(url, options) {
    ...
    var promise = new Promise(function(resolve, reject) {
        ...
        rsvp.hash(item.locks).then(function(){
            //and then pass the item along
            resolve(item);
        });
    }).
    then(meta_data_still_fresh).
    then(function(meta) {
        //meta data is still fresh
        item.locks.meta(); //release lock on metadata file
        return item;
    }, function(item) {
        //meta_data is stale or doesn't exist
        //download a new version of the source
        var promise = lock(source_path).then(function(source_lock) {
            item.locks.source = source_lock;
            return download(url, item.locks, conf);
        }).then(function(success) {
            //download succeeded
            item.locks.source(); //source lock no longer needed
