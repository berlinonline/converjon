...
function handle_source_file_path(source, item) {
    ...
    if (!base_path) {
        throw new Error("No base path configured for alias '"+alias+"'.");
    }

    var source_file_path = path.join(base_path, source_path);

    if (source_file_path.indexOf(base_path) !== 0) {
...
function image_request(url, req, res) {
    ...
    if (options.file) {
        try {
            handle_source_file_path(options.file, item);
            item.source_type = "file";
        } catch (e) {
            handle_error_response({error: e}, res);
            return;
        }
    } else if (options.url) {
        ...
        if (!check_url_whitelist(item.source, item.conf)) {
            handle_error_response({error: url_whitelist_error(new Error(item.source))}, res);
        }
    } else {
        handle_error_response({error: new Error("URL and file parameter are both missing! At least one is required.")}, res);
        return;
    }
    ...
    cache(item, options).then(function(item) {
        var ended = false;
        function end(ev) {
            if (!ended) {
