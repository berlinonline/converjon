...
var get_source_file_stat = function(path) {
    return new Promise(function(resolve, reject) {
...
var get_source = function(item) {
    ...
    then(function(dir_path) {
        switch (item.source_type) {
            ...
            case "file":
                return get_source_file_stat(item.source).then(function(stat) {
                    item.source_headers = item.conf.headers || {};
                    item.source_headers.date = stat.mtime.toUTCString();
                    console.log(item.source_headers.date)
