...
function analyze_result(item, headers) {
    ...
    logging.debug(id, "Requesting analysis");
...
function download_source(item) {
    ...
    return new Promise(function(resolve, reject) {
        ...
        logging.debug(id, "Starting request to:", url);
        response_stream = request.get(url, request_options);
        response_stream.on("error", function(err) {
            var message;

            logging.debug(id, 'Error on request:', err.message);
        ...
        response_stream.on("response", function(response) {
            if (response.statusCode > 200) {
                logging.debug(id, "Received non-200 response:", response.statusCode);
...
var get_source = function(item) {
    ...
    }, function(err) { logging.debug(item.id, "Meta dir creation failed"); }).
    then(function(dir_path) {
    ...
    }, function(err) { logging.debug(item.id, "Source dir creation failed"); }).
    then(function() {
        return analyze_result(item);
    }, function(err) { logging.debug(item.id, "Downloading or copying source failed."); });
