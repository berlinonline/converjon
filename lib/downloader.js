...
module.exports = function(req) {
    ...
    fsUtils.makeParentDirRecursive(downloadPath, function() {
        ...
        if (client === https) {
            download_options.rejectUnauthorized = config.rejectUnauthorized;
        }

        var auth_credentials = authentication.getCredentials(sourceUrl);
        if (auth_credentials) {
            logger.debug(req, "Authentication credentials found for request url.");
            download_options.auth = auth_credentials.username + ':' + auth_credentials.password;
        }
