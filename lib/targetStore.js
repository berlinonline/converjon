...
{
    res.setHeader('ETag', req.targetFingerprint);
    send(req, sendFilePath).maxage(config.maxAge * 1000).pipe(res);
};
...
module.exports = function(options) {
    ...
    return function(req, res, next) {
        ...
        var respond = function(filePath) {
            ...
            statEmitter.emit('Request::Success');
        };
        ...
        eventDispatcher.once(req.errorEventKey, function(errorCode, errorMessage) {
            ...
            statEmitter.emit('Request::Failed');
        });
        ...
        fs.stat(req.targetStorePath, function(err, stat) {
            if (stat) {
                ...
                if (fileAge <= config.maxAge * 1000) {
                    logger.debug(req, 'target store item is still valid.');
            ...
            logger.debug(req, 'target store item is stale or non existant.');

            fsUtils.makeParentDirRecursive(req.targetStorePath, function() {                
                converter(req);
            }, function() {
                logger.error(req, 'Could not create target store directory', req.targetStorePath);
