...
module.exports = function(req) {
    ...
    var childProcess = process.create('exiftool', ['-config', exiftoolConfigPath, req.sourceStorePath]);
    childProcess.onSuccess(function(data) {
        ...
        if (analysis.hasOwnProperty('Error')) {
            logger.error("Request", req.id, "Exiftool reported error", analysis.Error);
        ...
        if (!analysis.hasOwnProperty('width')) {
            logger.error("Request", req.id, "Essential EXIF data missing, unable to determine image dimensions");
