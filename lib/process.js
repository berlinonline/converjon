...
var Process = function(id, command, args, callback) {
    this.id = id;
...
Process.prototype = {
    run: function() {
        waitingProcesses.push((function() {
            ...
            logger.debug(this.id, 'Starting process: ', command);
            ...
            try {
                this.childProcess = child.exec(command, {
                    ...
                    logger.debug(this.id, 'Process ended: ', command);

                    if (error === null) {
                        this.success(stdout.toString('utf8'));
                        
                    } else {
                        logger.error(this.id, "Process:", command, "has error output", stderr.toString('utf8'));
                        this.error();
                    }
                }.bind(this));
            } catch (e) {
...
module.exports = {
    ...
    create: function(id, command, args) {
        logger.debug(id, 'Process requested', "running processes:", runningProcesses, "waiting processes:", waitingProcesses.length);
       return new Process(id, command, args);
    }
};
