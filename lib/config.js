...
exports.get = function(key) {
    ...
    configs.forEach(function(conf) {
        if (key && conf.urls && conf.urls.length) {
            conf.urls.forEach(function(pattern) {
                if (calmcard(pattern, key)) {
                    merged_config = merge(merged_config, conf);
                }
            });
        } else {
            // a config without URL restrictions is always applied
            merged_config = merge(merged_config, conf);
        }
    });

    //return a clone
    return merge(true, merged_config);
};
