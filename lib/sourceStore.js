...
module.exports = function(req) {
    ...
    logger.debug(req, 'looking up source store');
    ...
    fsUtils.makeParentDirRecursive(req.sourceStorePath, function() {
        fs.stat(req.sourceStorePath, function(err, stat) {
            if (stat) {
                ...
                if (fileAge <= config.maxAge * 1000) {
                    logger.debug(req, 'source store item is still valid.');
            ...
            logger.debug(req, 'source store item is stale or non existant.');

            eventDispatcher.once(req.downloadSuccessEventKey, function(sourceStorePath) {
                logger.debug(req, "download ready", sourceStorePath);
            ...
            eventDispatcher.once(req.downloadErrorEventKey, function(error) {
                logger.debug(req, "Download reported failure");
        ...
        logger.error(req, 'Could not create source store directory');
    });
};
